...
// void EKVStore_Local::get_key_local(uint64_t pid, ikey_t & key) {
...
EKVStore_Local::EKVStore_Local(Buffer * buf) : buf_(buf) {
    ...
    // cout << "INFO: EKVStore_Local = " << header_sz + entry_sz << " bytes " << std::endl
...
void EKVStore_Local::init(vector<Node> & nodes, uint64_t off, uint64_t slots_num, uint64_t buckets_num) {
    offset = off;
    num_slots = slots_num;
...
// void EKVStore_Local::get_property_local(uint64_t pid, value_t & val) {
//     ikey_t key;
...
// void EKVStore_Local::get_label_local(uint64_t pid, label_t & label) {
//     value_t val;
